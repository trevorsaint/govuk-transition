...
(function (Modules) {
  ...
  Modules.Feedback = function () {
    this.start = function ($element) {
      ...
      this.$fields = $element.find('.app-feedback__form-field')
      ...
      function setHiddenValuesNotUsefulForm (gaClientId) {
        ...
        if (document.querySelectorAll('.app-feedback__email-link#take-survey').length === 0) {
          that.$surveyWrapper.append('<a href="https://www.smartsurvey.co.uk/s/gov-uk-banner/?c=' + finalPathName + '&amp;gcl=' + gaClientId + '" class="app-feedback__email-link govuk-link" id="take-survey" target="_blank" rel="noopener noreferrer">Don’t have an email address?</a>')
        }
      }
      ...
      function toggleForm (formId) {
        ...
        if (formIsVisible) {
          that.$activeForm.find('.app-input').first().focus()
        } else {
      ...
      function showError (error) {
        var genericError = [
          '<h2 class="app-feedback__heading">',
